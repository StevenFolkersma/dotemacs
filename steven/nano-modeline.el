...
(defun nano-modeline-ispell-dictionary ()
  "Get the ispell dictionary name if flyspell-mode is active."
  (when (and (bound-and-true-p flyspell-mode) (bound-and-true-p ispell-current-dictionary))
    ispell-current-dictionary))

(defun nano-modeline-dict-short-name (dict)
  "Return a short name for a given ispell dictionary."
  (alist-get dict '(("en_GB" . "EN")
                    ("nl_NL" . "NL"))
             dict nil #'string=))


(defun nano-modeline-text-mode ()
  "Generate the modeline for text-mode without the branch but with the ispell dictionary if flyspell-mode is active."
  (let ((buffer-name (format-mode-line "%b"))
        (mode-name   (nano-mode-name))
        (ispell-dict (nano-modeline-ispell-dictionary))
        (position    (format-mode-line "%l:%c")))
    (nano-modeline-compose (nano-modeline-status)
                           buffer-name
                           (concat "(" mode-name
                                   (if ispell-dict (concat ", " (propertize (nano-modeline-dict-short-name ispell-dict) 'face 'bold)))
                                   ")" )
                           position)))
...
(defun nano-modeline ()
  ...
  '((:eval
     (cond ((nano-modeline-prog-mode-p)            (nano-modeline-default-mode))
           ((nano-modeline-text-mode-p)            (nano-modeline-text-mode))
