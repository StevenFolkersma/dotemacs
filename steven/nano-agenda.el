...
(defface nano-agenda-calendar-header-month
  '((t :inherit (nano-face-strong nano-face-subtle)))
...
(defface nano-agenda-calendar-selected
  '((t :inherit (nano-face-salient nano-face-strong)))
...
(defun nano-agenda--entry-format (entry)
  ...
  (let* ((time (nano-agenda--entry-time entry))
         ...
         (header-face   'default)
         (time-face     (if is-conflict 'error 'nano-face-faded))
    ...
    (concat " "
            prefix
            (propertize " │ " 'face 'nano-face-subtle)
            header
            tag-align
            tags)))
